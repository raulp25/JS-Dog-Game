...
export class Rolling extends State {
    ...
    handleInput(input){
        ...
        if (!input.includes('Enter') && !this.game.player.onGround()) this.game.player.setState(states.FALLING,1);
        if (!input.includes('Enter') && !this.game.player.onGround()) this.game.player.setState(states.FALLING,1);
        if (input.includes('Enter')  && input.includes('ArrowUp') &&  this.game.player.onGround()){
            this.game.player.vy -= 27;
        }
        if (input.includes('ArrowDown') && !this.game.player.onGround()) this.game.player.setState(states.DIVING, 0);
...
export class Diving extends State {
    ...
    handleInput(input){
        ...
        if (input.includes('Enter') && this.game.player.onGround()) this.game.player.setState(states.ROLLING,2);
    }
}
...
export class Hit extends State {
    ...
    handleInput(input){
        ...
        if (this.game.player.frameX >= 10 && !this.game.player.onGround()) this.game.player.setState(states.FALLING,1);
